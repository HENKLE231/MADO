...
class Core:
    def __init__(self):
        ...
        self.manga_name = config.get_config('manga_name')
        self.base_link = config.get_config('base_link')
        self.drive_url = config.get_config('drive_url')
        self.phone_dir = config.get_config('phone_dir')
        self.pc_dir = config.get_config('pc_dir')
        self.initial_chapter = config.get_config('initial_chapter')
        self.final_chapter = int(config.get_config('final_chapter'))
        try:
            self.final_chapter = int(self.final_chapter)
            self.chapter = int(self.initial_chapter)
        except:
            pass
        self.imgs_dir = config.get_config('imgs_dir')
        self.download_dir = config.get_config('download_dir')
        self.files_dir = config.get_config('files_dir')
        self.chapters_dir = config.get_config('chapters_dir')
        self.final_dir = config.get_config('final_dir')
        self.delete_chapters = config.get_config('delete_chapters') == 'True'
    ...
    @staticmethod
    def verify_outcome(app_instances, error, coords_to_close=None):
        """
            :param app_instances: (Array) Lista com instância da Queue, RootWindow, InfoWindow e se houver
            SeleniumManager.
            :param error: (String) Mensagem de erro.
            :param coords_to_close: (Array) Lista de listas com coordenadas das janelas para fechar c/ clique.
            Se há erro, o processo de download e transferência é interrompido.
        """
        if error:
            # Faz unpacking das instâncias do aplicativo.
            if len(app_instances) == 4:
                queue, root_window, info_window, selenium_ma = app_instances
                try:
                ...
                queue, root_window, info_window = app_instances

            # Se há coordenadas fecha janelas por clique.
            if coords_to_close:
                screen_ma = ScreenMa()
                screen_ma.close_windows_by_coords(coords_to_close)

            # Divide a mensagem de erro por linhas.
            error_lines = [line.strip() for line in error.split("\n") if len(line) > 0]

            # Interrompe o processo
            info_window.interrupt_process(queue, root_window, error_lines, 'Erro!')

    def download_chapters(self, queue, root_window, info_window):
        """
        ...
        # Atualiza a tela.
        info_window.show_info(['Iniciou.'], 'Download')

        # Instancia classes.
        selenium_ma = SeleniumMa()
        system_ma = SystemMa()
        app_instaces = [queue, root_window, info_window, selenium_ma]
        ...
        system_ma.delete_files(self.download_dir, [self.manga_name])
        ...
        info_window.show_info(['Obtendo links das imagens.', f'Progresso: {completion_percentage:.2%}'])

        while self.chapter <= self.final_chapter:
            # Abre navegador.
            selenium_ma.open_nav(page_load_strategy='eager', display='headless')

            # Abre o link.
            self.verify_outcome(
                app_instaces,
                selenium_ma.open_link(f'{self.base_link}{self.chapter}', queue)
            )

            # Obtem os links das imagens do mangá na página.
            self.verify_outcome(
                app_instaces,
                selenium_ma.get_imgs_urls(self.manga_name, self.chapter)
            )
